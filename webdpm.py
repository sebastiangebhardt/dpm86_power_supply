...
@app.post("/output")
def post_output(value: str):
    if value in ['1', 'on']:
        return {"on": dpm.output(dpm.P_ON)} # output 1/on -- turn the output on"
    else:
        return {"off": dpm.output(dpm.P_OFF)} # output 0/off/"else" -- turn the output off
...
@app.post("/voltage")
def post_voltage(value: float): # set the voltage target
...
@app.post("/current")
def post_voltage(value: float): # set the current target
...
@app.post("/const_voltage")
def post_const_voltage(value: float): # set constant voltage delivery
    return {"const_voltage": dpm.const(dpm.P_VOLTAGE)}

@app.post("/const_current")
def post_const_current(value: float): # set constant current delivery
